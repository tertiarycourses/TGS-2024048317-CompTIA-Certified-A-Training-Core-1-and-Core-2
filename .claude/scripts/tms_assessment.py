...
def _api_key():
    """Resolve the LMS service key: shell env, then project, then user level."""
    import os
    for name in ("LMS_TMS_API_KEY", "EXTERNAL_API_KEY_FOR_CLAWDBOT"):
        v = os.environ.get(name)
        if v:
            return v.strip()
    for path in (".claude/lms_tms_api_key",
        ...
        try:
            ...
            pass
    for path in (".claude/.env", ".env", os.path.expanduser("~/.claude/.env")):
        try:
            with open(path) as f:
