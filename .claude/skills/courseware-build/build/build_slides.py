#!/usr/bin/env python3
"""Generate the CompTIA A+ (Core 1 and Core 2) course slide deck (all-white Tertiary house style).

Design helpers are the same set used by the tertiary-course-slides skill that
produced the Tertiary house reference deck (cover, section, content, two_col, cards3,
...
def cover():
    ...
    # course badge (top-right) — official CompTIA A+ badge, else text fallback
    badge=_logo("comptia-aplus-badge.png")
    if badge:
        s.shapes.add_picture(badge,Inches(10.35),Inches(0.6),width=Inches(2.2))
    else:
        rect(s,Inches(10.35),Inches(0.6),Inches(2.25),Inches(1.28),BLUE)
        txt(s,Inches(10.35),Inches(0.72),Inches(2.25),Inches(0.56),
            [[("CompTIA A+",21,WHITE,True)]],align=PP_ALIGN.CENTER)
        txt(s,Inches(10.35),Inches(1.28),Inches(2.25),Inches(0.5),
            [[("CORE 1  ·  CORE 2",9.5,WHITE,True)],
             [("220-1101  ·  220-1102",8.5,WHITE,False)]],align=PP_ALIGN.CENTER,space=1)
...
def two_col(title,left,right,kicker=None,lhead="",rhead=""):
    ...
    # Size the body to the LONGER column so a full schedule cannot clip the panel.
    rows=max(len(left),len(right))
    bsz=16 if rows<=7 else (14 if rows<=9 else 12.5)
    gap=10 if rows<=7 else (7 if rows<=9 else 5)
    bullets(s,Inches(1.1),Inches(2.7),Inches(5.2),Inches(3.8),left,size=bsz,gap=gap)
    bullets(s,Inches(7.2),Inches(2.7),Inches(5.05),Inches(3.8),right,size=bsz,gap=gap,mcolor=TEAL); footer(s); return s
...
def process_map(title,stages,kicker=None,color=BLUE,synthesis=None,animate=True):
    ...
    y=Inches(2.28); ch=Inches(2.62) if synthesis else Inches(3.3)
    spids=[]
    for i,st in enumerate(stages):
        ...
        # The label band and the caption band NEVER overlap: the label owns the space
        # under the number badge, the caption owns a fixed band at the card foot.
        # Both auto-shrink to fit rather than truncating, so nothing is cut mid-word.
        lbl=" ".join(str(lbl).split())
        lsz=13 if len(lbl)<=24 else (12 if len(lbl)<=34 else 11)
        lb_top=int(y+Inches(1.00)); lb_h=Inches(0.96)
        txt(s,x+Inches(0.12),lb_top,cw-Inches(0.24),lb_h,
            [[(lbl,lsz,INK,True)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE,space=0)
        if detail:
            det=" ".join(str(detail).split())
            # Size the caption to the band it actually has, so it cannot spill past the
            # card bottom: roughly 26 chars per line at 9pt in this card width.
            cap_top=lb_top+lb_h
            cap_h=int(y+ch-Inches(0.10))-cap_top
            lines_avail=max(1,int(cap_h/Inches(0.17)))
            dsz=9 if len(det)<=26*lines_avail else (8.5 if len(det)<=30*lines_avail else 8)
            det=_ellipsis(det,int(27*lines_avail))
            txt(s,x+Inches(0.1),cap_top,cw-Inches(0.2),cap_h,
                [[(det,dsz,GREY,False)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.TOP,space=0)
...
trainer_slide("YOUR TRAINER",C.TRAINER,"Principal Trainer\nTertiary Infotech Academy Pte Ltd",
 [("Role","Principal Trainer, Tertiary Infotech Academy Pte Ltd"),
  ("Expertise","IT infrastructure support, networking, systems administration and cybersecurity."),
  ("Delivers","WSQ CompTIA courses including A+, Network+, Security+, Linux+ and Cloud+."),
...
content("Let's Know Each Other",[
 ...
 "Your current experience supporting PCs, laptops, mobile devices or networks.",
 "Whether you are working towards the CompTIA A+ certification exams.",
 "What you most want to be able to do confidently by the end of this course."],kicker="ICE-BREAKER")
...
tile_grid("Download Your Course Material",[
 ...
 ("3 · Open this course","Select 'CompTIA Certified A+ Training (Core 1 and Core 2)'."),
...
tile_grid("Learning Outcomes",[
 ...
 ("LO3 · Plan upgrades","Develop action plans for upgrades and propose improvement ideas based on user needs."),
 ("LO4 · Test and document","Test infrastructure systems and organise information for developing user guides.")],
 kicker="WHAT YOU'LL ACHIEVE",cols=2,size=15)
tile_grid("Course Outline — Nine Domains Across Five Days",
 [(f"Topic {t['code']} — {t['title']}  ({t['core']})", t["subtitle"]) for t in C.TOPICS],
 kicker=f"CORE 1 + CORE 2  ·  {len(ACTIVITIES)} HANDS-ON LABS",cols=1,size=12)
chart_slide("CompTIA A+ Core 1 (220-1101) Exam Weighting",
 ["Mobile Devices","Networking","Hardware","Virtualization & Cloud","HW/Network Troubleshooting"],
 [("Share of Core 1 exam (%)",[13,23,25,11,28])],
 kicker="EXAM DOMAIN WEIGHTING",accent=BLUE,kind="column",
 insight="Hardware and Network Troubleshooting is the largest Core 1 domain at 28%, and Hardware follows at 25% — more than half the Core 1 exam sits in these two domains, so that is where your revision time should go.",
 number_format='0"%"')
chart_slide("CompTIA A+ Core 2 (220-1102) Exam Weighting",
 ["Operating Systems","Security","Software Troubleshooting","Operational Procedures"],
 [("Share of Core 2 exam (%)",[28,28,23,21])],
 kicker="EXAM DOMAIN WEIGHTING",accent=TEAL,kind="column",
 insight="Core 2 is the most evenly weighted of the two exams — Operating Systems and Security tie at 28% each, and no domain falls below 21%, so none can safely be skipped.",
 number_format='0"%"')
two_col("Lesson Plan — 5 Days, 8 hours/day",[
 (f"Day 1 — {C.DAY_THEMES[1]}",0),
 ("Digital Attendance (AM) · Introductions · Learning Outcomes",1),
 ("Topic 1: Mobile Devices (Labs 1–4)",1),
 ("Topic 2: Networking (Labs 5–11)",1),
 (f"Day 2 — {C.DAY_THEMES[2]}",0),
 ("Topic 3: Hardware (Labs 12–18)",1),
 ("Topic 4: Virtualization and Cloud (Labs 19–21)",1),
 (f"Day 3 — {C.DAY_THEMES[3]}",0),
 ("Topic 5: Hardware and Network Troubleshooting (Labs 22–28)",1)],
 [(f"Day 4 — {C.DAY_THEMES[4]}",0),
 ("Topic 6: Operating Systems (Labs 29–36)",1),
 ("Topic 7: Security begins (Labs 37–40)",1),
 (f"Day 5 — {C.DAY_THEMES[5]}",0),
 ("Topic 7: Security concludes (Labs 41–44)",1),
 ("Topic 8: Software Troubleshooting (Labs 45–49)",1),
 ("Topic 9: Operational Procedures (Labs 50–54)",1),
 ("TRAQOM Survey · Digital Attendance (Assessment)",1),
 ("Final Assessment (WA + PP)",1)],
 kicker="SCHEDULE",lhead="Days 1–3 · Core 1",rhead="Days 4–5 · Core 2")
tile_grid("Your Lab Toolkit — Everything Runs in the Browser",
 [(name, desc) for (name,url,desc) in C.LAB_TOOLS],
 kicker="NO INSTALL REQUIRED",cols=1,size=12,accent=TEAL)
tile_grid("Lab Tool Links",
 [(name, url) for (name,url,desc) in C.LAB_TOOLS],
 kicker="BOOKMARK THESE NOW",cols=1,size=13,accent=VIOLET)
...
tile_grid("Assessment",[
 ("Written Assessment (WA)","Short-Answer Questions (SAQ) · 1 hour · open book. Tests the underpinning knowledge taught across the nine domains."),
 ("Practical Performance (PP)","Hands-on support, configuration and troubleshooting tasks · 1 hour · open book. Tests the ability you built in the labs."),
...
tile_grid("Practice Exam — Before You Sit the Real Thing",[
 ("Where", C.PRACTICE_EXAM["url"]),
 ("Which exam", C.PRACTICE_EXAM["name"]),
 ("Why it helps", C.PRACTICE_EXAM["note"]),
 ("How to use it","Sit it once cold to find your weak domains, revise those, then sit it again to confirm the gap has closed."),
 ("Core 1 target","Aim for a consistent pass across all five Core 1 domains before booking 220-1101."),
 ("Core 2 target","Aim for a consistent pass across all four Core 2 domains before booking 220-1102.")],
 kicker="EXAM PREPARATION  ·  exams.tertiaryinfotech.com",cols=2,size=14,accent=VIOLET)

# ---------------- CORE CONCEPTS ----------------
section("CORE CONCEPTS","The Foundations Behind Every Domain","")
big_statement("A+ is a troubleshooting certification.","Every domain in both exams ultimately asks the same question: given these symptoms, what has actually failed, and what do you do about it?","WHY THIS COURSE",color=BLUE)
process_map("The CompTIA Six-Step Troubleshooting Methodology",[
 ("Identify the problem","Question the user · what changed · back up first"),
 ("Establish a theory","Question the obvious · rank by likelihood"),
 ("Test the theory","Confirm or reject · escalate if needed"),
 ("Plan and implement","Vendor guidance · state the rollback"),
 ("Verify and prevent","Full functionality · preventive measures"),
 ("Document","Findings · actions · outcomes")],
 kicker="THE FRAMEWORK BEHIND EVERY TROUBLESHOOTING LAB",color=VIOLET,
 synthesis=("EXAMINABLE IN ORDER","The steps are tested in sequence. Documenting is step six and is not optional — an undocumented fix teaches the organisation nothing and is invisible at audit."))
compare_table("Core 1 vs Core 2 — What Each Exam Tests",
 ["Dimension","Core 1 (220-1101)","Core 2 (220-1102)"],
 [["Focus","Physical infrastructure and connectivity","Software, security and process"],
  ["Largest domain","Hardware & Network Troubleshooting (28%)","Operating Systems and Security (28% each)"],
  ["Typical task","Diagnose a machine that will not POST","Remove malware and harden the workstation"],
  ["Tools","Multimeter, cable tester, toner probe","Task Manager, MMC, command line"],
  ["Mindset","What component failed?","What changed, and is it safe?"]],
 kicker="TWO EXAMS, ONE CERTIFICATION",accent=BLUE,
 note="You must pass BOTH exams to earn the A+ certification. They are sat separately and can be taken in either order, though Core 1 first is the conventional path.")
decision_map("Is This a Hardware or a Software Problem?",
 "Does the fault survive\na clean boot from\nother media?",
 ("YES — suspect hardware","The fault follows the machine rather than the installed system. Move to component isolation: reseat, substitute a known-good part, and test the minimum configuration."),
 ("NO — suspect software","The hardware is capable, so the installed OS, a driver, an update or malware is responsible. Move to the recovery ladder starting at the least destructive rung."),
 kicker="THE FIRST FORK IN EVERY DIAGNOSIS",
 note="This single question saves more time than any other in support work — it halves the search space before you touch a screwdriver or a recovery tool.")
tile_grid("The Layers You Diagnose Through",[
 ("Physical","Is it powered, plugged in, and does the link light show carrier? Nothing above this layer works until this one does."),
 ("Addressing","Does the host have a valid IP, mask and gateway — or an APIPA address meaning DHCP never answered?"),
 ("Routing","Is there a default route, and does the gateway respond? This separates a local fault from an internet fault."),
 ("Name resolution","Does DNS resolve? A host that pings an IP but not a name has a DNS fault and nothing else."),
 ("Application","Is the service listening, is the port reachable, and is the firewall permitting it?"),
 ("The user","What did they actually do, and what changed? Most faults are a recent change rather than a spontaneous failure.")],
 kicker="ALWAYS WORK BOTTOM-UP",cols=2,size=13,accent=TEAL)
compare_table("Secure and Insecure Protocol Pairs — Know These On Sight",
 ["Insecure","Port","Secure replacement","Port","Why it matters"],
 [["Telnet","23","SSH","22","Telnet sends credentials in clear text"],
  ["FTP","20/21","SFTP","22","FTP sends credentials and data in clear text"],
  ["HTTP","80","HTTPS","443","HTTP has no encryption or server authentication"],
  ["SNMPv1/v2","161/162","SNMPv3","161/162","Only v3 encrypts and authenticates"],
  ["POP3","110","POP3S","995","Mail and credentials otherwise travel in clear"]],
 kicker="EXAMINABLE PORT PAIRS",accent=AMBER,
 note="Every insecure protocol on this list still exists in the field. Recognising it and naming its replacement is a direct exam objective and a real security finding.")
tile_grid("The Numbers You Must Simply Know",[
 ("Six","Steps in the CompTIA troubleshooting methodology"),
 ("Seven","Stages in the laser imaging process, and steps in malware removal"),
 ("3-2-1","Backup copies, media types, and off-site copies"),
 ("1, 6, 11","The only non-overlapping channels in the 2.4 GHz band"),
 ("100 m","Maximum twisted-pair copper run length"),
 ("4 GB","Maximum file size on a FAT32 volume"),
 ("169.254","The APIPA range that means DHCP failed"),
 ("2","Addresses reserved in every subnet — network and broadcast")],
 kicker="HIGH-YIELD EXAM FACTS",cols=2,size=14,accent=AMBER)
flow_h("How This Course Is Structured",[
 "Concepts for the domain",
 "Hands-on lab with real tools",
 "Verify your own result",
 "Recap against the exam objective",
 "Practice exam by domain"],kicker="CONCEPT → PRACTICE → VERIFY",color=TEAL)
...
for t in C.TOPICS:
    ...
    tile_grid(f"Topic {t['code']} at a Glance — {t['title']}",[
        ("Exam", f"CompTIA A+ {t['core']}"),
        ("Domain weighting", f"{t['exam_weight']}"),
        ("Share of course time", t["weighting"]),
        ("Hands-on labs", ", ".join(f"Lab {a['num']}" for a in TOPIC_ACTS[t["num"]])),
        ("Sub-topics", t["subtitle"]),
        ("Why it matters", "This domain is assessed in both the WSQ Written Assessment and the Practical Performance."),
    ], kicker=f"{t['core'].upper()}  ·  {t['exam_weight'].upper()}", cols=2, size=13, accent=VIOLET)
    cons=t["concepts"]
    CH=4
    chunks=[cons[i:i+CH] for i in range(0,len(cons),CH)]
    for ci,chunk in enumerate(chunks):
        suffix="" if ci==0 else f" ({ci+1})"
        tile_grid(f"Key Concepts — {t['title']}{suffix}", chunk,
                  kicker=f"TOPIC {t['code']}  ·  {t['exam_weight']}", cols=2, size=12)
    ...
    for a in acts:
        ...
        # Process-map chips carry a SHORT human step, never a shell command: a command
        # squeezed into a chip gets cut mid-token and is then wrong, not merely short.
        # The full instruction and the commands live in the Learner Guide and the lab folder.
        # The process map uses the lab's OWN authored `flow` — five short (stage, caption)
        # pairs written to fit a chip. Deriving them from the full instructions produced
        # mid-word truncation and split decimals ("Enter 192"), so the data carries them.
        stg=[(lbl, cap) for (lbl, cap) in a.get("flow", [])][:5]
        if len(stg)>=3:
        ...
        test_slide(a["title"], a["test"], kicker=f"LAB {a['num']} · VERIFY",
                   troubleshoot=[
                     ("A step won't run","Re-read the step in the Learner Guide — it carries the full detail the slide summarises."),
                     ("A browser tool won't load","Check the URL against the Lab Tool Links slide; all four tools run client-side and need no login."),
                     ("Killercoda has reset","The playground times out when idle. Reopen it and re-run the setup commands from the start of the lab."),
                   ])
    content(f"Recap — {t['title']}",
            ["You can now: "+a["objective"] for a in {x["objective"]:x for x in acts}.values()][:6],
            kicker=f"TOPIC {t['code']} RECAP · {t['exam_weight']}", size=15)

# ---------------- CLOSE ----------------
section("WRAP-UP","Course Summary & Next Steps","")
tile_grid("What You Achieved",[
 ("LO1 · Diagnosed and resolved","Applied the six-step methodology to hardware, network, OS and application faults, resolving root causes rather than symptoms."),
 ("LO2 · Troubleshot and configured","Configured SOHO networks, RAID, BIOS/UEFI, Windows security, Linux permissions and mobile device management."),
 ("LO3 · Planned upgrades","Produced memory, storage, power and backup upgrade plans justified against real requirements and constraints."),
 ("LO4 · Tested and documented","Verified every lab against a stated success criterion and produced tickets, runbooks and knowledge base articles.")],
 kicker="LEARNING OUTCOMES",cols=2,size=14)
tile_grid("Your Certification Path",[
 ("Step 1 · Core 1 (220-1101)","Mobile devices, networking, hardware, virtualization and cloud, hardware and network troubleshooting."),
 ("Step 2 · Core 2 (220-1102)","Operating systems, security, software troubleshooting and operational procedures."),
 ("Both required","You must pass both exams to earn the CompTIA A+ certification. They are sat separately."),
 ("Practice first",f"Sit the practice exam at {C.PRACTICE_EXAM['url']} and revise the domains where you scored lowest."),
 ("Rebuild the labs","Redo each lab from a blank sheet until the diagnostic sequence is automatic rather than looked up."),
 ("Keep the tools","The four browser tools and the Killercoda playground stay free — use them to keep the skills current.")],
 kicker="NEXT STEPS",cols=2,size=14,accent=TEAL)
tile_grid("Keep Practising — Your Lab Toolkit",
 [(name, url) for (name,url,desc) in C.LAB_TOOLS],
 kicker="FREE · NO INSTALL · KEEP USING THEM",cols=1,size=13,accent=VIOLET)
...
tile_grid("Assessment",[
 ("Written Assessment (SAQ)","1 hour · open book · short-answer knowledge questions across the nine domains."),
 ("Practical Performance (PP)","1 hour · open book · hands-on support, configuration and troubleshooting tasks."),
...
tile_grid("Digital Attendance (Mandatory)",[
 ...
 ("75% minimum","A minimum of 75% attendance is required to be eligible for assessment and funding.")],
 kicker="TRAQOM · SSG DIGITAL ATTENDANCE",cols=2,size=15)
big_statement("Thank You!","You can now diagnose, configure, secure and document the hardware, networks, operating systems and mobile devices an IT support role depends on.","GOOD LUCK IN YOUR EXAMS",color=TEAL)
