...
def check(pdf):
    ...
    if not is_key:
        ...
        # 7
        # Match on word boundaries: a bare substring search flags ordinary prose
        # such as "upgrading the network" as the "Grading" block.
        for banned in ("Trainee Information", "Instructions to Candidate", "Grading"):
            if re.search(rf"\b{banned}\b", all_txt, re.I):
